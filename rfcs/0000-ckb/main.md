...
The current generation blockchain technologies are increasingly difficult to meet the demand of today's real world applications.

Bitcoin is the first blockchain network in the world, designed as a peer to peer cash ledger. The Bitcoin ledger is the system state maintained by the Bitcoin network. The minimum storage unit on the Bitcoin ledger is UTXO. Users can use wallets to spend current UTXOs, generate new UTXOs, and package them into transactions to send to the Bitcoin network for validation and consensus. The UTXOs have both the cash amount and ownership information expressed with lock scripts. Users have to provide proper unlocking data to spend UTXOs. Due to data structure and Bitcoin script's limitations, it's difficult to use the Bitcoin ledger to record other types of assets and data on the Bitcoin blockchain. Solutions like Colored Coins, Meta Coins or Bitcoin hard forks are possible, but they are expensive and inflexible to implement.

Ethereum brought us a general purpose computation blockchain platform with the introduction of smart contracts. The Ethereum ledger is the system state maintained by the Ethereum network. Ethereum's ledger consists of the global states of all accounts. Smart contracts are accounts that have code stored inside, together with a 256 bit K/V store. Users can start two types (???) of transactions on Ethereum: the first type is to create a contract and deploy it on the blockchain; the second type is to send input data to a specific contract account. This executes the code stored in the contract and updates contract state. Ethereum's smart contract design provides more computation power, allows more flexibility, and solves some of Bitcoin's problems. But it still has limitations:

* Difficult to scale: Ethereum's design focuses on the state machine's events (see figure 1). With a Turing complete scripting language and transactions containing state transitions inputs (instead of the states themselves), it's difficult for Ethereum's full nodes to tell dependencies between transactions. This makes it difficult for the nodes to to process transactions in parallel and potential sharding solutions need to take care of data availability issues.

* Uncertainty in result states: in Ethereum, the states of the contracts are updated by the contract code, which can depend on the execution context (such as the current block height). Users won't know the exact execution result when they start the transactions.
...
Current blockchain technologies also pursue complete decentralization, requiring the full nodes in the network to be equal peers. This imposes constraints on the design of the systems, making it harder for them to meet the demands of real world applications. The hardware cost of running full nodes becomes ever more expensive with the inflation of on-chain states. There'll be less and less users who're willing to run full nodes. At the same time, users are increasingly relying on mobile devices and mobile apps to access the internet, instead of the desktop based web apps. This exaberates the problems of equal peer design of full nodes. Having multiple types of blockchain nodes is going to be the norm of the future.

Based on all above, we conceived and designed Nervos CKB with a complete decoupled new paradigm for DApps. Nervos CKB supports more general purposed computation and storage, comes with better performance and more balanced economic incentives, and is more friendly to mobile devices. We hope Nervos CKB can become the common knowledge base of the 7.6 billion people, and the foundation of all types of distributed applicaitons.
...
Nervos CKB (CKB for short) is designed as general purpose common knowledge base ([Appendix: Common Knowledge Base](#appendix-common-knowledge-base)). The CKB network consists of three types of nodes:

* Archive Nodes: Full nodes on the CKB network. They validate new blocks and transactions, relay and broadcast blocks, and archive all historical transactions on the CKB. Archive Nodes can increase the overall robustness of the system, and provide query services for historical data.
* Consensus Nodes: Consensus participating nodes on the CKB network. Consensus Nodes listen to new transactions and package them to blocks, and achieve consensus on new blocks. Consensus Nodes don't have to store all the transaction history.
* Light Nodes: Users interact with the CKB network with Light Nodes. They store only very limited data, and can run on desktop computers or mobile devices.

CKB nodes together form a peer to peer distributed network to relay and broadcast blocks and transactions. Consensus Nodes run a hybrid consensus protocol ([hybrid consensus](#hybrid-consensus)), to generate consensus on newly created blocks after a certain time interval. The new block will be acknoledged by all nodes and added to the end of the blockchain. The transactions in the new block will update CKB's states.
...
With those five components, we completely decouple distributed applications into computation, storage and Identity. On top of this, computation can be further divided to state generation (Generator) and state validation (Validator); Storage (Cell) is made more generic to support any structured (Type) data. The DApps in the CKB can use Types to define the appropriate data structure and put application data in Cells. The application logic is implemented with Generators, and the state validation logic is implemented with Validators. Generators run on the client side to generate new states, which get packaged to transactions and broadcast to the entire network. The full nodes in the network first validate the identity of the submitter of the transaction, then validate the new states in the transaction with Validators. After the validation passes, new states are stored into the CKB.

The CKB's design of data flow (??) and economic incentives is based on states - transactions store new states, instead of the events that trigger the state machine. Therefore, the CKB blockchain stores state data directly, and states are synchronized together with the synchronization of the blockchain, with no need for extra state synchronization protocols. This reduces complexity of the system and increases system availability. A DApp's states are stored in Cells. The Generators and Validators are pure functions without internal states, relying entirely on the inputs to produce outputs, and can be combined to form complicated logics. CKB's DApps use a computation paradigm similar to the Lambda Calculus, which can achieve the same computation capability as the Turing machine.
...
State generation and validation are separated in the CKB. They can use different algorithms, or the same algorithm.
...
* Certainty in transaction output: Certainty in transaction output is one of the core pursuits of distributed systems. Certainty in transaction latency (see [Hybrid Consensus](#hybrid-consensus)) has seen a lot of attention, but certainty in transaction output hasn't seen much discussions. If new states are generated on the nodes, a transaction's creator can't be certain about its execution context, and it could generate unexpected outputs. In CKB, the users generate new states on the client side. They can confirm with the new states before propagating it to the network. The users can be certain with the transaction outcome: whether the transaction passes validations and gets accepted by the network, or the validation process fails without state changes. (see diagram 2)

* Parallelism: If new states are generated on the nodes, the nodes won't know the state dependencies of transactions beforehand.  In CKB, because transactions include the old states and the new states, nodes can see dependency relationships between transactions (see [Transaction](#transaction)). Independent transactions can be processed in parallel in many ways, such as on different CPU cores or sent to different shards. Parallelism is the key solution to blockchain scalability problems.
...
There are two sources of trust for data in the CKB: the first type is when the data is independently verifiable, therefore trust is built-in, and the second type is when the data is endorsed by Identities in the system. Data in CKB has to include the following elements:

* The data itself
* Validation logic of the data in its application domain
* The Identity of the submitter of the data

Cells are the smallest data units in the CKB, and they include the following properties:
...
Cell's lock scripts are executed on CKB's VM. When updating data or transferring ownership, users need to provide necessary proof as inputs of the lock scripts. User authorization can be proved if the lock scripts return true, and the user is allowed to perform operations with the cell.

The lock scripts represent authorizations of the cells. The scripts can represent a single user, as well as a multisig or more complicated schemes. Cells come with privacy support. Users can manage their cells with pseudonym using different lock scripts. Cell's owners and rightful users can be the same or different users, which means users don't have to own cells to interact with the CKB. This lowers the barrier of entry of the system and encourages adoption.
...
The data schema defines the data structure of the cell so that the validator can interpret and interact with the data. Validators are verifying programs that run on full nodes, in the virtual machine that CKB provides. A validator uses the transaction's inputs and outputs as inputs ([Transaction](#transaction)), and returns a boolean value on whether the validation is successful. The creation, update and destroy of cells can use different validators.

### Index

Users can set up indices when they define a data schema. The CKB provides more support for indexed properties, such as conditional query commands and aggregate functions in validators or `owner_lock` / `data_lock` scripts. For example, to start a capped crowd sale, the starter of the sale can create an Identity Cell ([Identity](#identity)), and uses conditional query (???) and aggregate functions to tell whether the crowd sale's goal has been achieved.

## Identity

Identity is a system Type. Users can create identity cells that belong to themselves, and they can be used for cell's `data_lock`/`owner_lock` scripts. A cell's transfer and update need its identity cell's `data_lock` unlock script (see Figure 3)

Identity in the Nervos CKB is generalized identity that could represent any individuals or machines. Identity cells can store any data or digest related to identities. Identity is the foundation of the Nervos Identity Protocol (see Nervos Identity Protocol Paper for details). With NIP, the Nervos network brings in the CA certificates system to be compatible with the current PKI system. Users can express their own identity, and distributed applications can be built on top of identities. (???)

![Figure 3. Identity Cell](fig3.png)
> Figure 3. Identity Cell

The cell model is a more generic data storage model. Both the UTXO model and the account model can express relationships between assets and their owners. The UTXO model defines ownership based on assets, and the account model defines ownership based on owners. The UTXO model makes the transaction more clear, but its lack of explicit accounts makes its already inexpressive scripts harder to use. There's also no way to record account metadata such as authorizations. The account model is easy to understand, and can support authorizations and identities well, but it's difficult to process transactions in parallel. The cell model, and the identity model on top of it takes the best of both models and provides support for more generic data.

## Transaction

Transactions express the creations and updates of cells. Users can use transactions to create a cell, or update one or more cells in the CKB. A transaction includes the following:

* deps: a set of dependencies, including the P1 cells needed for validation and the user inputs.
* inputs: a set of inputs, including all the transferred / updated P1 cells and their unlock scripts.
* outputs: a set of outputs, including all the newly created P1 cells.

Because cells are immutable, we won't modify the cells directly, but will create a new cell. All of versions of cells are linked together, forming a cell chain: a cell creation transaction creates its first version, and the updates become its historical versions. The head of a cell chain is its latest version. The CKB is the superset of all cell chains. The set of all cell heads is the current version of the CKB.

The `deps` and `inputs` in CKB transactions make it easier for nodes to determine dependency relationships between transactions. (see figure 4). Transactions can include many types of cells for atomic, cross type operations.

The design of CKB cell model and transactions is friendly to light clients. Since all the states are in the blocks, the block synchronization also accomplishes state synchronization. Light clients only need to synchronize blocks, and don't need to perform state transition computations. If we only stored events in blocks, we would've needed full nodes to also support state synchronization. This can be difficult for large deployments, because the incentive to do so is not defined within blockchain protocol. We define state synchronization in the protocol itself, and this makes light nodes and full nodes leveled peers, leading to a more robust and decentralized system. 
