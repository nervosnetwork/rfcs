...
- Best Chain: a chain with the most accumulated PoW, and starting with common genesis block which nodes agree with consensus.
...
2. Downloading Block: Get and validate the complete block. Transactions in ancestor blocks is not required.
...
3. Connected: A block succeeds in stage Connecting Header, and all its ancestor blocks are in status of connected, downloaded or accepted.
4. Downloaded: A block succeeds in stage Downloading Block, and all its ancestor blocks are in status of downloaded or accepted.
5. Accept: A block succeeds in stage Accepting Block, and all its ancestor blocks are in status of accepted.

Block status is propagated from the previous block to the later ones. Using the list index number above, the status number of a block is always less then or equal to its parent block. Here are conditions, if a block is invalid, all of its descendant blocks must be invalid. The cost of every step for synchronization is higher than the previous one and every step may fail. In this scenario, work will be wasted if child block enters a later status before parent block, and parent block is approved to be Invalid later.
...
1. If Bob's Best Chain Tip is in Alice's Best Header Chain, the latest common block will be Bob's Best Chain Tip and there is no block headers for Bob to send.
...
- The status of the parent block of the first block are not Unknown in the local Status Tree, which means Orphan Block will not be processed in synchronizing.
...
The figure above is the Status Tree of Alice, after synchronized with Bob, Charlie, Davis, Elsa. The observed Best Chain Tip of each peers are also annotated in the figure.

If the Unknown status block is considered not on the Status Tree, new blocks in the status of Connected or Invalid will be extended to the leaves of the Status Tree during Connecting Header. As a result, Connecting Header stage explores and extends status tree.
...
When downloading a branch, earlier blocks should be downloaded firstly due to the dependency of blocks; and the download should be done concurrently from different peers to utilize full bandwidth. A sliding window can be applied to solve the problem.
...
The figure above is an example of an 8 length sliding window. At the beginning, the downloadable block range from 3 to 10. After block 3 is downloaded,  the window will move to block 5 because block 4 has already been downloaded in advance (as the figure above illustrated).
...
Downloading Block will update status of blocks in the best connected chain, from Connected to Downloaded or Invalid.
...
When the local Best Chain Tip changes, the node should push an announcement to peers. The best header with most cumulative work sent to each peer should be recorded, to avoid sending duplicate blocks in announcement and sending blocks only peer doesn't known.This does not only record headers sent for new blocks, but also the ones sent as response in stage Connecting Header.
...
2. Another new block is added the best chain, before Alice have a chance to send the headers. In this case, the last two blocks of Best Chain need to be sent.
...
- In other cases, just send block header list with a upper limit on number of blocks to send. For example, if the limit is 8 and there are 8 or more blocks need to be announced, only the latest 7 blocks will be announced.
