...
There are two rights with cells - ownership and usage. Cell’s owner_lock specifies its ownership, the right to transfer cell capacity; Cell’s data_lock specifies its usage right, the right to update the cell’s data. CKB allows users to transfer cell capacity all at once, or transfer only a fraction of a cell's capacity(e.g. a cell with capacity=10 becomes two cells with capacity=5), and both kinds of transfer leads to cell creation . The rate of increase of CKB’s total capacity is decided by Consensus Participating Rate and Liquid Voting.
